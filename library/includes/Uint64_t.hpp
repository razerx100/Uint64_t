...
public:
    inline constexpr Uint64_t() : m_storage{ 0u } {}
    inline constexpr Uint64_t(std::uint64_t value) : m_storage{ value } {}

    inline operator std::uint64_t() const noexcept { return m_storage; }
    inline explicit operator std::uint32_t() const noexcept
    { return static_cast<std::uint32_t>(m_storage); }
    inline explicit operator std::uint16_t() const noexcept
    { return static_cast<std::uint16_t>(m_storage); }
    inline explicit operator std::uint8_t() const noexcept
    { return static_cast<std::uint8_t>(m_storage); }

    inline constexpr Uint64_t& operator=(std::uint64_t value) noexcept
    {
        m_storage = value;
    ...
    inline constexpr auto operator<=>(const Uint64_t& other) const noexcept
    {
